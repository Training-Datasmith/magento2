...
class Redis_Lua_Helper
{
     ...
     */
    public function clean_by_tag_conditional(string $tag_key, string $prefix, string $condition = 'all'): int
    {
        if (!$this->is_enabled()) {
            return 0;
        }
        $sha = $this->load_script(self::SCRIPT_CLEAN_BY_TAG_CONDITIONAL);
        try {
            $result = $this->redis->eval_sha($sha, [$tag_key, $prefix, time(), $condition], 2);
            return (int) $result;
        } catch (\Redis_Exception $e) {
            // Fallback: script not loaded, try eval
            return (int) $this->redis->eval(self::SCRIPT_CLEAN_BY_TAG_CONDITIONAL, [$tag_key, $prefix, time(), $condition], 2);
        }
    }
    /**
     ...
     * @return bool
     */
    public function atomic_save_with_tags(string $cache_key, string $value, int $ttl, array $new_tag_keys, string $reverse_index_key): bool
    {
        if (!$this->is_enabled()) {
            return false;
        }
        $sha = $this->load_script(self::SCRIPT_ATOMIC_SAVE_WITH_TAGS);
        $argv = array_merge([$value, $ttl], $new_tag_keys);
        try {
            $result = $this->redis->eval_sha($sha, array_merge([$cache_key, $reverse_index_key], $argv), 2);
            return (bool) $result;
        } catch (\Redis_Exception $e) {
            // Fallback: script not loaded
            return (bool) $this->redis->eval(self::SCRIPT_ATOMIC_SAVE_WITH_TAGS, array_merge([$cache_key, $reverse_index_key], $argv), 2);
        }
    }
    /**
     ...
     */
    public function garbage_collect(string $pattern, string $tag_prefix, int $batch_size = 1000): array
    {
        if (!$this->is_enabled()) {
            return [0, 0];
        }
        $sha = $this->load_script(self::SCRIPT_GARBAGE_COLLECT);
        $total_deleted = 0;
        $iterations = 0;
        $cursor = '0';
        do {
            try {
                $result = $this->redis->eval_sha($sha, [$pattern, $tag_prefix, $batch_size, $cursor], 2);
                $cursor = $result[0];
                $deleted = $result[1];
        ...
        return [$total_deleted, $iterations];
    }
    /**
     ...
     */
    public function clear_all_indices(string $namespace, int $batch_size = 1000): int
    {
        if (!$this->is_enabled()) {
            return 0;
        }
        $sha = $this->load_script(self::SCRIPT_CLEAR_ALL_INDICES);
        // Build patterns
        $tag_pattern = 'cache:tags:' . $namespace . '*';
        $reverse_pattern = 'cache:id_tags:' . $namespace . '*';
        $all_ids_key = 'cache:all_ids';
        try {
            $result = $this->redis->eval_sha($sha, [$tag_pattern, $reverse_pattern, $all_ids_key, $batch_size], 3);
            return (int) $result;
        } catch (\Redis_Exception $e) {
            // Fallback: run script directly
            try {
                $result = $this->redis->eval(self::SCRIPT_CLEAR_ALL_INDICES, [$tag_pattern, $reverse_pattern, $all_ids_key, $batch_size], 3);
                return (int) $result;
            } catch (\Redis_Exception $e) {
     ...
     */
    private function load_script(string $script): string
    {
        $hash = hash('sha256', $script);
        if (isset($this->script_shas[$hash])) {
            return $this->script_shas[$hash];
        }
        try {
            $sha = $this->redis->script('load', $script);
            $this->script_shas[$hash] = $sha;
            return $sha;
        } catch (\Redis_Exception $e) {
            throw new \RuntimeException('Failed to load Lua script: ' . $e->get_message(), 0, $e);
        }
    }
    /**
     ...
     * @return void
     */
    public function clear_script_cache(): void
    {
        $this->script_shas = [];
    }
}