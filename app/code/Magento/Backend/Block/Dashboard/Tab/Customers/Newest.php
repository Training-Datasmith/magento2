...
class Newest extends \Magento\Backend\Block\Dashboard\Grid
{
     ...
     * @param array $data
     */
    public function __construct(\Magento\Backend\Block\Template\Context $context, \Magento\Backend\Helper\Data $backend_helper, \Magento\Reports\Model\Resource_Model\Customer\Collection_Factory $collection_factory, array $data = [])
    {
    ...
    {
        parent::_construct();
        $this->set_id('customersNewestGrid');
    }
    /**
     * {@inheritdoc}
     */
    ...
    {
        $collection = $this->_collection_factory->create()->add_customer_name();
        $store_filter = 0;
        ...
        $collection->add_orders_statistics($store_filter)->order_by_customer_registration();
    ...
    protected function _prepare_columns()
    {
        ...
        $this->add_column('orders_avg_amount', ['header' => __('Average'), 'sortable' => false, 'type' => 'currency', 'currency_code' => $base_currency_code, 'index' => 'orders_avg_amount', 'renderer' => \Magento\Reports\Block\Adminhtml\Grid\Column\Renderer\Currency::class, 'header_css_class' => 'col-avg', 'column_css_class' => 'col-avg']);
        $this->add_column('orders_sum_amount', ['header' => __('Total'), 'sortable' => false, 'type' => 'currency', 'currency_code' => $base_currency_code, 'index' => 'orders_sum_amount', 'renderer' => \Magento\Reports\Block\Adminhtml\Grid\Column\Renderer\Currency::class, 'header_css_class' => 'col-total', 'column_css_class' => 'col-total']);
    ...
    {
        return $this->get_url('customer/index/edit', ['id' => $row->get_id()]);
    }
}